...
export default function Proveedores() {
  ...
  function abrirEditar(p: Proveedor) { setEditando(p); setForm({ nombre: p.nombre, nit: p.nit ?? "", telefono: p.telefono ?? "", contacto: p.contacto ?? "", direccion_id: p.direccion_id ? String(p.direccion_id) : "", activo: p.activo }); setErrores({}); openModal(); }
  ...
  async function borrar(p: Proveedor) { if (!confirm(`¿Eliminar "${p.nombre}"?`)) return; await eliminar(p.id); }

  return (
    <>
      <PageMeta title="Proveedores | DATALAN" description="Terceros" />
      <div className="flex flex-col gap-4 mb-5 sm:flex-row sm:items-center sm:justify-between">
        <h1 className="text-xl font-semibold text-gray-800 dark:text-white/90">Proveedores</h1>
        <div className="flex gap-3">
          ...
          {puede("proveedores.crear") && <Button size="sm" onClick={abrirNuevo}>+ Nuevo</Button>}
        </div>
      </div>
            ...
            <TableBody className="divide-y divide-gray-100 dark:divide-gray-800">
              ...
              : items.map((p) => (<TableRow key={p.id}>
                ...
                <TableCell className="px-5 py-4 text-right text-theme-sm">
                  {puede("proveedores.editar") && <button onClick={() => abrirEditar(p)} className="mr-3 text-brand-500 hover:underline">Editar</button>}
                  {puede("proveedores.eliminar") && <button onClick={() => borrar(p)} className="text-error-500 hover:underline">Eliminar</button>}
                </TableCell>
              </TableRow>))}
            </TableBody>
          </Table>
      ...
      <Modal isOpen={isOpen} onClose={closeModal} className="max-w-lg p-6 m-4">
        <h2 className="mb-4 text-lg font-semibold text-gray-800 dark:text-white/90">{editando ? "Editar proveedor" : "Nuevo proveedor"}</h2>
