...
export default function UnidadesMedida() {
  ...
  function abrirEditar(p: UnidadMedida) {
    setEditando(p); setForm({ nombre: p.nombre, abreviatura: p.abreviatura }); setErrores({}); openModal();
  }
  ...
  async function borrar(p: UnidadMedida) {
  ...
  return (
    <>
      <PageMeta title="Unidades de Medida | DATALAN" description="Catálogo de unidades de medida" />
      <div className="flex flex-col gap-4 mb-5 sm:flex-row sm:items-center sm:justify-between">
        <h1 className="text-xl font-semibold text-gray-800 dark:text-white/90">Unidades de Medida</h1>
        <div className="flex gap-3">
          ...
          {puede("unidades.crear") && <Button size="sm" onClick={abrirNuevo}>+ Nuevo</Button>}
        </div>
      </div>
              ...
              {cargando ? (
                ...
                items.map((p) => (
                  <TableRow key={p.id}>
                    ...
                    <TableCell className="px-5 py-4 text-gray-500 text-theme-sm dark:text-gray-400">{p.abreviatura}</TableCell>
                    <TableCell className="px-5 py-4 text-right text-theme-sm">
                      {puede("unidades.editar") && <button onClick={() => abrirEditar(p)} className="mr-3 text-brand-500 hover:underline">Editar</button>}
                      {puede("unidades.eliminar") && <button onClick={() => borrar(p)} className="text-error-500 hover:underline">Eliminar</button>}
                    </TableCell>
                  </TableRow>
                ))
              )}
      ...
      <Modal isOpen={isOpen} onClose={closeModal} className="max-w-lg p-6 m-4">
        <h2 className="mb-4 text-lg font-semibold text-gray-800 dark:text-white/90">
          {editando ? "Editar unidad" : "Nueva unidad de medida"}
        </h2>
        <form onSubmit={guardar} className="space-y-4">
          ...
          <div><Label>Abreviatura *</Label><Input value={form.abreviatura} onChange={(e) => setForm({ ...form, abreviatura: e.target.value })} />{errores.abreviatura && <p className="mt-1 text-xs text-error-500">{errores.abreviatura[0]}</p>}</div>
