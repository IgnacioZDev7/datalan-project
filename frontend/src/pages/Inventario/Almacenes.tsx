            ...
            <TableBody className="divide-y divide-gray-100 dark:divide-gray-800">
              ...
              : items.map((p) => (<TableRow key={p.id} className="hover:bg-gray-50 dark:hover:bg-white/[0.03]">
                ...
                <TableCell className="px-5 py-4">
                  <Acciones>
                    {puede("almacenes.editar") && <BotonAccion titulo="Editar" color="brand" onClick={() => abrirEditar(p)}>{IconoEditar}</BotonAccion>}
                    {puede("almacenes.eliminar") && <BotonAccion titulo="Eliminar" color="error" onClick={() => borrar(p)}>{IconoEliminar}</BotonAccion>}
                  </Acciones>
                </TableCell>
              </TableRow>))}
            </TableBody>
